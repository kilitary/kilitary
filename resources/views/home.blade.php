...
@section('content')
    <div class="container">
        ...
        @if (isset($gdiSelected) && $gdiSelected)
            <h3> global defense initiative selected (in {{ XRandom::scaled(0,1) ? 'case' : 'state' }} of {{$chanceOf}}
                )</h3>
        @else
            <h3> brotherhood of nod selected (in {{ XRandom::scaled(0,1) ? 'case' : 'state' }} of {{$chanceOf}})</h3>
        @endif
